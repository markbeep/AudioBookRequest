...
async def generate_abs_metadata(book: Audiobook, dest_path: str, mam_result: Optional[_Result] = None):
    """
    ...
    if mam_result:
        ...
        if mam_result.tags:
            metadata["genres"] = list(set(metadata["genres"] + [t.strip() for t in mam_result.tags.split(",")]))

    file_path = os.path.join(dest_path, "metadata.json")
    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(metadata, f, indent=4, ensure_ascii=False)

async def generate_opf_metadata(session: Session, book: Audiobook, dest_path: str, mam_result: Optional[_Result] = None):
    """
