...
class QbittorrentClient:
    def __init__(self, session: Session, host: Optional[str] = None, port: Optional[int] = None, username: Optional[str] = None, password: Optional[str] = None):
        ...
        self.username = username if username is not None else download_client_config.get_qbit_user(session)
        self.password = password if password is not None else download_client_config.get_qbit_pass(session)
    ...
    async def _login(self, client: aiohttp.ClientSession) -> tuple[bool, int, str]:
        ...
        try:
            async with client.post(url, data=data, headers=self.headers) as resp:
                ...
                logger.error("qBittorrent login failed", status=resp.status, url=url, response=text)
                return False, resp.status, text
        except Exception as e:
            logger.error("qBittorrent login exception", error=str(e), url=url)
            return False, 0, str(e)

    async def add_torrent(self, torrent_data: str, is_magnet: bool = True, category: Optional[str] = None, save_path: Optional[str] = None, tags: Optional[List[str]] = None) -> bool:
        if not self.host:
            return False
        ...
        async with aiohttp.ClientSession() as client:
            ...
            elif default_path := download_client_config.get_qbit_save_path(self.session):
            ...
            try:
                async with client.post(url, data=data, cookies=self.cookies) as resp:
                    if resp.status == 200:
                        logger.info("Successfully added torrent to qBittorrent", path=final_save_path)
                        return True
                    logger.error("Failed to add torrent to qBittorrent", status=resp.status, text=await resp.text())
                    return False
            except Exception as e:
                logger.error("qBittorrent add_torrent exception", error=str(e))
                return False

    async def get_torrents(self, category: Optional[str] = None, filter: Optional[str] = None) -> List[Dict[str, Any]]:
        if not self.host:
            return []
